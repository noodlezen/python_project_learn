...
class User(Basic, object):
    ...
    def input_db(self):
        ...
        my_API_mysql.SQL('Web_Shop_User_DB', sql)
...
def register_user():
    user_apellation = raw_input(my_proces_text.color(
        'Plaes input User apellation:\n', 'green', 'l'))
    ob = User(user_apellation, 'vip')

    while(1):
        feature_apellation = raw_input(my_proces_text.color(
            'Plaes input feature apellation:\n', 'blue', 'l'))
        num_apellation = raw_input(my_proces_text.color(
        ...
        tmp = raw_input(my_proces_text.color('Plaes check!   y/n?\n', 'red', 'f'))
        if tmp == 'y':
