...
__RULE_WORDBOOK = {
        ...
        '密码': ('^(?=.*\d).{8,20}$' , '密码必须以数字的组合，允许大小写字母，特殊字符。长度8-20字节。'),
...
def check_record_rule(p_feature, p_num):
    # print p_feature
    pattern_array = __RULE_WORDBOOK.get(p_feature)
    if pattern_array != None:
        pattern = re.compile(pattern_array[0])
        result = pattern.findall(p_num) #.findall以列表形式返回
        if result:
            return True
        else:
