    ...
    def update_datebase(self):
        ...
        for keys in res_dict.iterkeys():
            if res_dict[keys] != self.wrong_dict[keys]:
                self.mysql.update_single('userwrong', keys, self.wrong_dict[keys], 'wrong_id' , self.ID)
                update_conut += 1
        return update_conut
...
class Meta(Basic, object):
    ...
    def load_wrong(self):
        wrong_dict = self.mysql.xload_userwrong(self.user_id, self.ID)
        if wrong_dict != False:
            ...
            self.wrong = Wrong(name, genre, toplimit, expiry_date, user_id, meta_id, wrong_id, count, created, wrong_dict)
            return True
        else:
            return False
    ...
    def update_datebase(self):
        ...
        for keys in res_dict.iterkeys():
            if res_dict[keys] != self.meta_dict[keys]:
                self.mysql.update_single('usermeta', keys, self.meta_dict[keys], 'meta_id' , self.ID)
                update_conut += 1
        return update_conut
...
class User(Basic, object):
    ...
    def load_meta(self, meta_name):
        ...
        if meta_dict != False:
            ...
            self.meta = Meta(name, genre, value, types, user_id, meta_id, modify, created, meta_dict)
            self.meta_list.append(self.meta)
            self.meta_dict[name] = self.meta  # 添加到用户字典
            return True
        else:
    ...
    def update_datebase(self):
        ...
        for keys in res_dict.iterkeys():
            if res_dict[keys] != self.user_dict[keys]:
                self.mysql.update_single('users', keys, self.user_dict[keys], 'user_id' , self.ID)
                update_conut += 1
        return update_conut
